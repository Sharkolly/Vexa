...
const List = ({ isLoading, shopData }: ShopProductsType) => {
  return (
    <>
      {isLoading && !shopData ? (
        <Loader />
        ...
        {/* <div className="flex flex-co flex-wrap justify-between  gap- gap-y-8"> */}
          {shopData.map((item: ProductType) => (
            <div className="flex flex-row gap-4 border-b-1.5 rounded-lg  border-b-slate-200 shadow-md md:p-5 p-2 py-4" key={item._id}>
              <Link to={`/products/product/${item._id}`} className="relative">
                <img
                  src={item?.image}
                  alt="product"
